// A mean reversion trading agent.
...
int main(int argc, char* argv[]) {
    if (argc != 2) {  // wrong number of command line arguments
        std::cerr << "Usage:\n\tmean_reversion <config>" << std::endl;
        std::cerr << "Example:\n\tmean_reversion config.json" << std::endl;
        return 1;
    }
    try {
        ...
        Strategies::MeanReversion strategy(feed_context, context, config);
