...
#include "strategies/liquidity_consumer.hpp"
#include <fstream>
#include <thread>

int main(int argc, char* argv[]) {
    if (argc != 2) {  // wrong number of command line arguments
        std::cerr << "Usage:\n\tliquidity_consumer <config>" << std::endl;
        std::cerr << "Example:\n\tliquidity_consumer config.json" << std::endl;
        return 1;
    }
    try {
        ...
        Strategies::LiquidityConsumer strategy(feed_context, context, config);
