// A multi-cast data feed receiver that outputs raw data.
...
#include "data_feed/handlers/echo.hpp"
#include <nlohmann/json.hpp>
#include <fstream>

using DataFeed::Receiver;
using DataFeed::Handlers::Echo;

int main(int argc, char* argv[]) {
    if (argc != 2) {  // wrong number of command line arguments
        std::cerr << "Usage:\n\techo <config>" << std::endl;
        std::cerr << "Example:\n\techo config.json" << std::endl;
        return 1;
    }
    try {
        ...
        Echo printer;
        Receiver<Echo> receiver(context, listen, group, port, printer);
        // start the service
        context.run();
