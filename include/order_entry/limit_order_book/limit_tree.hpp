...
#ifndef ORDER_ENTRY_LIMIT_ORDER_BOOK_LIMIT_TREE_HPP
#define ORDER_ENTRY_LIMIT_ORDER_BOOK_LIMIT_TREE_HPP
...
struct LimitTree {
    ...
    Volume volume = 0;
    /// the feed for the limit tree
    DataFeed::Sender* feed = nullptr;
    /// the side character for this side of the tree
    static constexpr DataFeed::Side data_feed_side = DataFeed::bool_to_side(static_cast<bool>(side));
    /// the side character for the opposing side of the tree
    static constexpr DataFeed::Side data_feed_side_market = DataFeed::bool_to_side(!static_cast<bool>(side));

    /// @brief Initialize a new limit tree with a feed.
    ///
    /// @param feed_ the feed to create the limit tree feed with
    ///
    LimitTree(DataFeed::Sender* feed_ = nullptr) : feed(feed_) { }
    ...
    void limit(Order* order) {
        ...
        // send the quote to the feed
        if (feed != nullptr)
            feed->send<DataFeed::Messages::AddOrder>(order->uid, order->price, order->quantity, data_feed_side);
    }
    ...
    /// @brief Cancel an order in the limit tree.
    ///
    /// @param order an order that exists in the limit tree
    ///
    inline void cancel(Order* order) {
        ...
        // send the cancellation to the feed
        if (feed != nullptr)
            feed->send<DataFeed::Messages::DeleteOrder>(order->uid);
    }

    /// Perform a market order on the given limit tree.
    ///
        ...
        while (best != nullptr && can_match<side>(best->key, order->price)) {
            ...
            if (match->quantity >= order->quantity) {  // current match can fill
                ...
                if (match->quantity == order->quantity) {  // limit order filled
                    ...
                    // send trade to the feed
                    if (feed != nullptr)
                        feed->send<DataFeed::Messages::Trade>(match->uid, match->price, match->quantity, data_feed_side_market);
                    ...
                    // send trade to the feed
                    if (feed != nullptr)
                        feed->send<DataFeed::Messages::Trade>(match->uid, match->price, order->quantity, data_feed_side_market);
                }
                // set the order quantity to 0 since it's filled
                order->quantity = 0;
                return;
    ...
    inline Volume volume_at(Price price) const {
        if (limits.count(price))  // limit exists at price
...
#endif  // ORDER_ENTRY_LIMIT_ORDER_BOOK_LIMIT_TREE_HPP
