// A liquidity consumer trading strategy.
...
#ifndef STRATEGIES_LIQUIDITY_CONSUMER_HPP
#define STRATEGIES_LIQUIDITY_CONSUMER_HPP
...
/// @brief The liquidity consumer strategy logic.
/// @details
/// \image html liquidity-consumer/liquidity-consumer.svg "Liquidity Consumer Agent Algorithm"
/// \image latex liquidity-consumer/liquidity-consumer.pdf "Liquidity Consumer Agent Algorithm"
///
class LiquidityConsumer {
 private:
    /// a type for the receiver using this class as an event handler
    typedef DataFeed::Receiver<LiquidityConsumer> DataFeedReceiver;
    ...
    inline void sell_order() {
        ...
        if (volume == 0) return;
        if (volume > size) volume = size;
        // std::cout << "placing sell order for " << volume << " shares" << std::endl;
        // cast volume to order entry because it is normally a larger type.
        // it is guaranteed within range clamping with size above
        client.send<OrderEntry::Messages::OrderRequest>(
            OrderEntry::Messages::ORDER_PRICE_MARKET,
            static_cast<OrderEntry::Quantity>(volume),
    ...
    inline void buy_order() {
        ...
        if (volume == 0) return;
        if (volume > size) volume = size;
        // std::cout << "placing buy order for " << volume << " shares" << std::endl;
        // cast volume to order entry because it is normally a larger type.
        // it is guaranteed within range clamping with size above
        client.send<OrderEntry::Messages::OrderRequest>(
            OrderEntry::Messages::ORDER_PRICE_MARKET,
    ...
    inline void start_strategy() {
        ...
        timer.async_wait([this](const std::error_code& error) {
            if (error) {  // TODO: handle error code
                std::cout << "LiquidityConsumer::start_strategy - " << error << std::endl;
                return;
            }
...
#endif  // STRATEGIES_LIQUIDITY_CONSUMER_HPP
