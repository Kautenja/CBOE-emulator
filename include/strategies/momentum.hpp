...
#ifndef STRATEGIES_MOMENTUM_HPP
#define STRATEGIES_MOMENTUM_HPP
...
/// @brief The momentum trader strategy logic.
/// @details
/// \image html momentum/momentum.svg "Momentum Agent Algorithm"
/// \image latex momentum/momentum.pdf "Momentum Agent Algorithm"
///
class Momentum {
 private:
    /// a type for the receiver using this class as an event handler
    typedef DataFeed::Receiver<Momentum> DataFeedReceiver;
    ...
    /// the rate of change (RoC) of midpoint price
    Maths::RateOfChange<OrderEntry::Price> roc;
    /// the maximal size for the orders to place
    OrderEntry::Quantity max_size;
    ...
    inline void start_strategy() {
        ...
        timer.async_wait([this](const std::error_code& error) {
            if (error) {  // TODO: handle error code
                std::cout << "Momentum::start_strategy - " << error << std::endl;
                return;
            }
    ...
    void do_strategy() {
        // update the rate of change based on the current midpoint price
        roc.process(receiver.get_book().last_price());
        // determine the quantity as product of the rate_of_change and the
        // "wealth" of the trader, i.e., their capital
        OrderEntry::Quantity size = std::abs(roc.get_rate_of_change() * client.get_capital());
        static constexpr OrderEntry::Quantity min_size = 0;
        size = std::clamp(size, min_size, max_size);
        // place an order
        if (roc.get_rate_of_change() >= decision_boundary)  // increasing, buy
            client.send<OrderEntry::Messages::OrderRequest>(
                OrderEntry::Messages::ORDER_PRICE_MARKET,
                size,
                OrderEntry::Side::Buy
            );
        else if (roc.get_rate_of_change() <= -decision_boundary)  // decreasing, sell
            client.send<OrderEntry::Messages::OrderRequest>(
                OrderEntry::Messages::ORDER_PRICE_MARKET,
                size,
 ...
 public:
    ...
    Momentum(
        ...
        is_running(false),
        roc(options["strategy"]["history_size"].get<uint32_t>()),
        max_size(options["strategy"]["max_size"].get<OrderEntry::Quantity>()),
        decision_boundary(options["strategy"]["decision_boundary"].get<double>()) {
            ...
            // set the capital and shares for the client
            client.set_capital(options["order_entry"]["capital"].get<OrderEntry::ClientCapital>());
            client.set_shares(options["order_entry"]["shares"].get<OrderEntry::ClientShares>());
        }
